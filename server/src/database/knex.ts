...
export default class KnexDatabaseAdapter extends Database {
  ...
  private async createTableIfNotExists(
    tableName: string,
    tableBuilderCallback: (tableBuilder: Knex.CreateTableBuilder) => any
  ) {
  ...
  public async getChats(userID: string): Promise<any[]> {
    return await this.knex(tableNames.chats).where("user_id", userID).select();
  }
  ...
  public async createShare(
    ...
    await this.knex(tableNames.shares).insert({
      id,
      user_id: userID,
      created_at: new Date(),
    });
  ...
  public async deleteChat(userID: string, chatID: string): Promise<any> {
    await this.knex.transaction(async (trx) => {
      await trx(tableNames.chats)
        .where({ id: chatID, user_id: userID })
        .delete();
      await trx(tableNames.messages)
        .where({ chat_id: chatID, user_id: userID })
        .delete();
      await trx(tableNames.deletedChats).insert({
        id: chatID,
        user_id: userID,
        deleted_at: new Date(),
      });
  ...
  protected async loadYDoc(userID: string) {
    ...
    if (updates.length) {
      ...
      await this.knex.transaction(async (trx) => {
        await trx(tableNames.yjsUpdates).insert({
          user_id: userID,
  ...
  public async saveYUpdate(userID: string, update: Uint8Array): Promise<void> {
    ...
    await this.knex(tableNames.yjsUpdates).insert({
      user_id: userID,
